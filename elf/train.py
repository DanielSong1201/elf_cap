...
ELF_SOURCE_ROOT = os.path.dirname(os.path.abspath(__file__))
if ELF_SOURCE_ROOT not in sys.path:
    sys.path.insert(0, ELF_SOURCE_ROOT)
...
from utils.checkpoint_utils import (
    save_checkpoint, load_checkpoint, load_model_weights, find_latest_checkpoint,
)
...
def run_training(config, *, force_cpu: bool = False):
    ...
    if config.init_checkpoint:
        load_model_weights(config.init_checkpoint, model)
        log_for_0(
            f"Initialized model weights from {config.init_checkpoint}; "
            "optimizer, scheduler, step, and epoch start fresh"
        )
