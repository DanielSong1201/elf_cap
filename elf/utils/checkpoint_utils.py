...
def load_model_weights(checkpoint_path: str, model) -> int:
    """Load model parameters only for fine-tuning initialization.

    Unlike ``load_checkpoint``, this intentionally does not restore optimizer,
    scheduler, RNG, step, or epoch state from the source task.
    """
    log_for_0(f"Loading ELF initialization weights from {checkpoint_path}...")
    ...
    if ckpt is None:
        raise ValueError(
            f"Failed to initialize model from {checkpoint_path}. "
            f"Tried: {'; '.join(errors)}"
        )
    inner_model = unwrap_model(model)
    inner_model.load_state_dict(ckpt["params"], strict=True)
    step = int(ckpt.get("step", 0))
    log_for_0(f"Loaded {loaded_from} model parameters (source step {step})")
    return step
